...
class DatabaseOperations:
    ...
    @staticmethod
    def create_file_upload(db: Session, filename: str, file_content: bytes, 
                             user_id: Optional[int] = None, 
                             storage_path: Optional[str] = None) -> FileUpload:
        """Create a new file upload record"""
        try:
            file_hash = hashlib.sha256(file_content).hexdigest()
            query = db.query(FileUpload).filter(FileUpload.file_hash == file_hash)
            if user_id:
        ...
        try:
            ...
            for col_analysis in analysis_data.get('column_analysis', []):
                column_analysis = ColumnAnalysis(
                    ...
                    suggestions=col_analysis['suggestions'],
